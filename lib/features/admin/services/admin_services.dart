...
class AdminServices {
  void sellProduct({
    ...
    try {
      final cloudinary = CloudinaryPublic("mst1993-hb", "ml_default");
      ...
      httpErrorHandle(
          ...
          context: context,
          onSuccess: () {
            showSnackBar(context, "Product add Successful");
  ...
  void deletedProduct(
    ...
    final userProvider = Provider.of<UserProvider>(context, listen: false);
    try {
      http.Response res = await http.post(Uri.parse("$uri/admin/add-delete"),
          headers: {
            'Content-Type': 'application/json,charset=UTF-8',
            'x-auth-token': userProvider.user.token
          },
